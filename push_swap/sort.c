...
/*   Updated: 2023/09/18 16:44:14 by string           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	search_rangevalue_operation(s_node **a_stack, s_node **b_stack,
...
int	search_value_operation_pa(s_node **a_stack, s_node **b_stack,
		size_t search_value)
{
	size_t	prev_dist_to_destination;
	size_t	next_dist_to_destination;
...
void	sort_seven_or_more(size_t n, s_node **a_stack, s_node **b_stack)
{
	...
	while ((*a_stack)->coord_num != NIL && a_size > 3)
	{
		...
		if ((*b_stack)->coord_num != NIL && (*b_stack)->next->coord_num != NIL
			&& (*b_stack)->coord_num < (*b_stack)->next->coord_num)
		{
			if ((*a_stack)->next->coord_num != NIL
	...
	while ((*b_stack)->coord_num != NIL)
	{
		...
		if (isprev_push)
		{
			if ((*b_stack)->coord_num != NIL
				&& (*b_stack)->next->coord_num != NIL
				&& (*b_stack)->coord_num < (*b_stack)->next->coord_num)
				ss(a_stack, b_stack);
			else
				sa(a_stack, 1);
